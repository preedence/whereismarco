...
map.on("load", () => {
  ...
  // Sorgente per le foto (GeoJSON, usato anche come sorgente vettoriale)
  ...
  // Duomo di Milano come symbol layer, con toggle dimensioni
  map.loadImage("images/duomo.png", (error, image) => {
    ...
    // Layer simbolo del Duomo - inizialmente piccolo
    map.addLayer({
    ...
    // Stato interno: se l'icona è ingrandita o no
    let duomoEnlarged = false;

    // Aggiorna dimensione in base allo zoom (quando NON è in stato "enlarged")
    ...
    // Primo settaggio coerente con zoom iniziale
    updateDuomoSize();

    // Click: toggle ingrandimento / dimensione normale
    map.on("click", "duomo-start-layer", () => {
      if (!duomoEnlarged) {
        // Ingrandisci e centra
        duomoEnlarged = true;
        ...
        // Riclick: torna alla dimensione in base allo zoom
        duomoEnlarged = false;
        updateDuomoSize();
      }
    });

    // Zoom end: se fai zoom out sotto una soglia, torna piccolo
    map.on("zoomend", () => {
      const z = map.getZoom();
      if (z < 10) {
        duomoEnlarged = false;
        ...
        // Se non è ingrandito, mantieni la logica normale
        updateDuomoSize();
      }
    });

    // (Opzionale) durante lo zoom, aggiorna se non ingrandito
    map.on("zoom", () => {
      if (!duomoEnlarged) {
        updateDuomoSize();
      }
